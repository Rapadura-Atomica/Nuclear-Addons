...
class RollViewModalOperator(bpy.types.Operator):
    """Arraste para rolar a visualização ou clique para desfazer todas as rolagens anteriores."""
...
class ArrangeModalOperator(bpy.types.Operator):
    """
    Arraste para a esquerda/direita para organizar os traços para baixo/para cima
    """
...
class OffsetTool(bpy.types.WorkSpaceTool):
    ...
    bl_label = "2D Offset"
    bl_description = (
        "Deslocar/inserir a forma dos traços selecionados arrastando com o mouse. "
...
def register_viewport_tools():
    ...
    if kc:
        ...
        kmi = km.keymap_items.new(
            RollViewModalOperator.bl_idname,
            type='LEFT_ALT',
            value='PRESS',
            ctrl=True,
            shift=False,
