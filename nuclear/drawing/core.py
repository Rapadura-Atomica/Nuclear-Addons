...
def create_gpencil_object(
    ...
    """
    Cria um novo objeto Grease Pencil v3 no Blender 4.4 usando o operador correto.
    """
    # Sair do modo atual se necessário
...
# nuclear/drawing/core.py
def get_active_gp_object() -> Optional[bpy.types.Object]:
    """Get the active Grease Pencil object with proper validation."""
    active_obj = bpy.context.active_object
    
    if not active_obj:
        return None
    
    if active_obj.type != "GREASEPENCIL":
        return None
    
    # Verificação adicional para dados GP válidos
    if not hasattr(active_obj, 'data') or not active_obj.data:
        return None
        
    # Verificar se é um dos tipos de Grease Pencil suportados
    if not isinstance(active_obj.data, (bpy.types.GreasePencil, bpy.types.GreasePencilv3)):
        return None
...
def set_gpencil_mode_safe(
    ...
    with context.temp_override(
        ...
        if mode not in ("OBJECT", "EDIT_GREASE_PENCIL"):
...
def refresh_quick_edit_gizmo(context: bpy.types.Context):
    ...
    if (
        tool is not None
        and tool.mode == "PAINT_GREASE_PENCIL"
