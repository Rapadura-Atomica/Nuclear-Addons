...
def get_active_gp_object(context=None) -> bpy.types.Object:
    """
    Get the active Grease Pencil object in the scene.
    
    Args:
        context (bpy.types.Context): Optional Blender context. If None, uses bpy.context.
    
    Returns:
        bpy.types.Object: The active Grease Pencil object or None if not found/invalid.
    """
    ctx = context or bpy.context
    
    # Verifica se há um objeto ativo e se é do tipo Grease Pencil
    if not ctx.active_object:
        return None
    
    if ctx.active_object.type != 'GREASEPENCIL':
        return None
    
    # Verificação adicional para garantir que os dados são válidos
    if not hasattr(ctx.active_object.data, 'layers'):
        return None
