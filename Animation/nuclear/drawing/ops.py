...
from nuclear.drawing.core import (
    clear_parent,
...
from nuclear.gpu_utils import OverlayDrawer, ui_scaled
...
def quick_edit_poll(context: bpy.types.Context) -> bool:
    ...
    if not getattr(context.active_object, "type", None) == "GREASEPENCIL":
        return False
...
def execute(self, context: bpy.types.Context):
    ...
    if mode == "ACTIVE":
        ...
        if use_current_frame:
            try:
                frame = layer.frames.lookup(context.scene.frame_current)
            except KeyError:
                return {"CANCELLED"}
        else:
            frames_sorted = sorted(layer.frames, key=lambda f: f.frame_number)
            frame_numbers = [f.frame_number for f in frames_sorted]

            # Procura a posição mais próxima do frame_offset
            idx = max(
                bisect(
                    frame_numbers,
