...
class LegacyPointCollection:
    ...
    def add(self, count):
        ...
        real_count = count
        if self._drawing.attributes['.nuclear_new'].data[self._stroke._index].value:
            self._drawing.attributes['.nuclear_new'].data[self._stroke._index].value = False
            real_count = count - 1
    ...
    def weight_get(self, vertex_group_index, point_index):
        ...
        offset = self._stroke.get_offset()
        group_name = f'.nuclear_weight_proxy_{vertex_group_index}'
...
class LegacyStrokeRef:
    ...
    def update_index(self):
        ...
        hashes = self._drawing.attributes['.nuclear_hash'].data
...
class LegacyStrokeCollection:
    ...
    def __init__(self, frame):
        ...
        hash_attr = frame.drawing.attributes.new(".nuclear_hash", 'INT', 'CURVE') if '.nuclear_hash' not in frame.drawing.attributes else frame.drawing.attributes['.nuclear_hash']
        ...
        if '.nuclear_new' not in frame.drawing.attributes:
            frame.drawing.attributes.new(".nuclear_new", 'BOOLEAN', 'CURVE')
    ...
    def __getitem__(self, key):
        if key >= len(self):
            raise IndexError()
        hash_attr = self._drawing.attributes['.nuclear_hash']
    ...
    def new(self):
        ...
        self._drawing.attributes['.nuclear_hash'].data[key].value = random.randint(1, 2 ** 28)
        self._drawing.attributes['.nuclear_new'].data[key].value = True
        ...
        return LegacyStrokeRef(self._drawing, self._drawing.attributes['.nuclear_hash'].data[key].value, key)
...
def register_alternative_api_paths():
    ...
    if bpy.app.version >= (4, 3, 0):
        ...
        bpy.types.GreasePencilFrame.nuclear_strokes = property(lambda self: LegacyStrokeCollection(self))
    else:
        bpy.types.GPencilFrame.nuclear_strokes = property(lambda self: self.strokes)
...
def unregister_alternative_api_paths():
    if bpy.app.version >= (4, 3, 0):
        ...
        delattr(bpy.types.GreasePencilFrame, "nuclear_strokes")
    else:
        delattr(bpy.types.GPencilFrame, "nuclear_strokes")